...
import iconTime from "@/utils/iconTime";
import WeatherDetails from "@/components/WeatherDetails";
import { fromUnixTime } from "date-fns";
import convertWindSpeed from "@/utils/convertWindSpeed";
import convertVisability from "@/utils/convertVisability";
  ...
  const { isPending, error, data, refetch } = useQuery<WeatherData>({
    ...
    queryFn: async () => {
      const { data } = await axios.get(
        `https://api.openweathermap.org/data/2.5/forecast?q=${place}&appid=1daea76ad85a0e91a469be3e0602c5a1&cnt=56`
      );
      return data;
    },
  });
          ...
          <>
            <section className="space-y-4">
                ...
                <Container className="gap-10 px-6 items-center">
                  ...
                  <div className="flex flex-col px-4">
                    <span className="text-5xl">
                      {convertTmp(date?.main.temp ?? 0)}°
                    </span>
                    <p className="text-xs space-x-1 whitespace-nowrap">
                      <span>Feels Like</span>
                      <span>{convertTmp(date?.main.feels_like ?? 0)}°</span>
                    </p>
                    <p className="text-xs space-x-2">
                      <span>{convertTmp(date?.main.temp_min ?? 0)}°↓ </span>
                      <span> {convertTmp(date?.main.temp_max ?? 0)}°↑</span>
                    </p>
                  </div>
                  ...
                  <div className="flex gap-10 sm:gap-16 overflow-x-auto w-full justify-between">
                    {data?.list.map((d, i) => (
                      ...
                      >
                        ...
                        <WeatherIcon
                          iconName={iconTime(d.weather[0].icon, d.dt_txt)}
                        />
                        <p>{convertTmp(d?.main.temp ?? 0)}°</p>
                      </div>
                    ))}
                  </div>
                </Container>
              ...
              <div className="flex gap-4">
                ...
                <Container className="bg-yellow-300/80 px6 gap4 justify-between overflow-x-auto">
                  <WeatherDetails
                    visability={convertVisability(date?.visibility ?? 10000)}
                    ...
                    windSpeed={convertWindSpeed(date?.wind.speed ?? 10000)}
                  />
                </Container>
              </div>
            </section>
            ...
            <section className="flex w-full flex-col gap-4">
              ...
              {firstDateForEachDate.map((d, i) => (
                <ForecastWeatherDetail
                  ...
                  visability={`${convertVisability(d?.visibility ?? 10000)}`}
  ...
  return (
    <section className="space-y-8">
      <div className="space-y-2 animate-pulse">
        <div className="flex gap-1 text-2x1 items-end">
          <div className="h-6 w-24 bg-gray-300 rounded"></div>
          <div className="h-6 w-24 bg-gray-300 rounded"></div>
        </div>
        {/* time wise temperture skeleton */}
